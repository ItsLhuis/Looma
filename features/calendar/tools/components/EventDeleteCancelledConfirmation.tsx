...
type EventDeleteCancelledConfirmationProps = {
  eventData: EventDeleteConfirmationData
}

function EventDeleteCancelledConfirmation({ eventData }: EventDeleteCancelledConfirmationProps) {
  ...
  return (
    <Card
      ...
      aria-label={`Event Delete Cancelled: ${eventData.title}`}
    >
      ...
      <CardContent className="flex h-full flex-col gap-6 wrap-break-word">
        ...
        <div className="bg-error/10 border-error/20 rounded-md border p-3">
          <Typography affects={["muted", "small"]} className="text-center">
            This event was not deleted. You can ask me to delete it again if needed.
          </Typography>
        </div>
      </CardContent>
    </Card>
...
export { EventDeleteCancelledConfirmation }
