...
type TaskCreationCancelledConfirmationProps = {
  taskData: CreateTaskToolInput
}

function TaskCreationCancelledConfirmation({ taskData }: TaskCreationCancelledConfirmationProps) {
  ...
  return (
    <Card
      ...
      aria-label={`Task Creation Cancelled: ${taskData.title}`}
    >
      <CardHeader className="flex items-start justify-between gap-3">
        <div className="space-y-1">
          ...
          <Typography affects={["muted", "small"]}>Task creation was cancelled by user</Typography>
        </div>
      ...
      <CardContent className="flex h-full flex-col gap-6 wrap-break-word">
        ...
        <div className="bg-error/10 border-error/20 rounded-md border p-3">
          <Typography affects={["muted", "small"]} className="text-center">
            This task was not created. You can ask me to create it again if needed.
          </Typography>
        </div>
      </CardContent>
    </Card>
...
export { TaskCreationCancelledConfirmation }
