...
type TaskUpdateCancelledConfirmationProps = {
  taskData: UpdateTaskToolInput
}

function TaskUpdateCancelledConfirmation({ taskData }: TaskUpdateCancelledConfirmationProps) {
  ...
  return (
    <Card
      ...
      aria-label={`Task Update Cancelled: ${taskData.title}`}
    >
      <CardHeader className="flex items-start justify-between gap-3">
        <div className="space-y-1">
          ...
          <Typography affects={["muted", "small"]}>Task update was cancelled by user</Typography>
        </div>
      ...
      <CardContent className="flex h-full flex-col gap-6 wrap-break-word">
        ...
        <div className="bg-error/10 border-error/20 rounded-md border p-3">
          <Typography affects={["muted", "small"]} className="text-center">
            This task was not updated. You can ask me to update it again if needed.
          </Typography>
        </div>
      </CardContent>
    </Card>
...
export { TaskUpdateCancelledConfirmation }
