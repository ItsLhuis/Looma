...
type MemoryCreatedConfirmationProps = {
  memoryData: CreateMemoryToolInput & { id: string }
}

function MemoryCreatedConfirmation({ memoryData }: MemoryCreatedConfirmationProps) {
  return (
    <Card
      className={cn("flex flex-col", getActionStatusClasses("success"))}
      aria-label={`Memory Created: ${memoryData.title}`}
    >
      <CardHeader className="flex items-start justify-between gap-3">
        <div className="space-y-1">
          ...
          <Typography affects={["muted", "small"]}>Memory has been successfully created</Typography>
        </div>
...
export { MemoryCreatedConfirmation }
