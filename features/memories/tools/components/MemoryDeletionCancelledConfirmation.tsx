...
type MemoryDeletionCancelledConfirmationProps = {
  memoryData: DeleteMemoryToolInput
}

function MemoryDeletionCancelledConfirmation({
  memoryData
}: MemoryDeletionCancelledConfirmationProps) {
  return (
    <Card
      className={cn("flex flex-col", getActionStatusClasses("cancelled"))}
      aria-label={`Memory Deletion Cancelled: ${memoryData.title}`}
    >
      <CardHeader className="flex items-start justify-between gap-3">
        <div className="space-y-1">
          ...
          <Typography affects={["muted", "small"]}>
            Memory deletion was cancelled by user
          </Typography>
        </div>
      ...
      <CardContent className="flex h-full flex-col gap-6 wrap-break-word">
        ...
        <div className="bg-error/10 border-error/20 rounded-md border p-3">
          <Typography affects={["muted", "small"]} className="text-center">
            This memory was not deleted. You can ask me to delete it again if needed.
          </Typography>
        </div>
      </CardContent>
    </Card>
...
export { MemoryDeletionCancelledConfirmation }
