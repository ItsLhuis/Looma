...
type MemoryUpdatedConfirmationProps = {
  memoryData: UpdateMemoryToolInput
}

function MemoryUpdatedConfirmation({ memoryData }: MemoryUpdatedConfirmationProps) {
  return (
    <Card
      className={cn("flex flex-col", getActionStatusClasses("success"))}
      aria-label={`Memory Updated: ${memoryData.title}`}
    >
      <CardHeader className="flex items-start justify-between gap-3">
        <div className="space-y-1">
          ...
          <Typography affects={["muted", "small"]}>Memory has been successfully updated</Typography>
        </div>
...
export { MemoryUpdatedConfirmation }
