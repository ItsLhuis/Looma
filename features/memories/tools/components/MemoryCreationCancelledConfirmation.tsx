...
type MemoryCreationCancelledConfirmationProps = {
  memoryData: CreateMemoryToolInput
}

function MemoryCreationCancelledConfirmation({
  memoryData
}: MemoryCreationCancelledConfirmationProps) {
  return (
    <Card
      className={cn("flex flex-col", getActionStatusClasses("cancelled"))}
      aria-label={`Memory Creation Cancelled: ${memoryData.title}`}
    >
        ...
        <div className="space-y-1">
          <div className="flex items-center gap-2">
            ...
            <Typography variant="h5" className="line-clamp-2 leading-tight">
              Memory Creation Cancelled
            </Typography>
          </div>
          <Typography affects={["muted", "small"]}>
            Memory creation was cancelled by user
          </Typography>
        </div>
      ...
      <CardContent className="flex h-full flex-col gap-6 wrap-break-word">
        ...
        <div className="bg-error/10 border-error/20 rounded-md border p-3">
          <Typography affects={["muted", "small"]} className="text-center">
            This memory was not created. You can ask me to create it again if needed.
          </Typography>
        </div>
      </CardContent>
    </Card>
...
export { MemoryCreationCancelledConfirmation }
