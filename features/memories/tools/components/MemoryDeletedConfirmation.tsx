...
type MemoryDeletedConfirmationProps = {
  memoryData: DeleteMemoryToolInput
}

function MemoryDeletedConfirmation({ memoryData }: MemoryDeletedConfirmationProps) {
  return (
    <Card
      className={cn("flex flex-col", getActionStatusClasses("success"))}
      aria-label={`Memory Deleted: ${memoryData.title}`}
    >
      <CardHeader className="flex items-start justify-between gap-3">
        <div className="space-y-1">
          ...
          <Typography affects={["muted", "small"]}>Memory has been permanently deleted</Typography>
        </div>
      ...
      <CardContent className="flex h-full flex-col gap-6 wrap-break-word">
        ...
        <div className="flex items-center gap-2">
          ...
          <Typography affects={["muted", "small"]}>
            Deleted: {formatDateForDisplay(new Date())}
          </Typography>
        </div>
      </CardContent>
    </Card>
...
export { MemoryDeletedConfirmation }
